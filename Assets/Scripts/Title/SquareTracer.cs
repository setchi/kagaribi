...
public class SquareTracer : AbstractSquareManager {
	public GameObject player;
	public PatternRoutines patternRoutines;
	Queue<GameObject> targetPositionQueue = new Queue<GameObject>();
	...
	IEnumerator StartTrace() {
		while (true) {
			if (targetPositionQueue.Count > 0) {
				var peekPos = targetPositionQueue.Peek().transform.position;
				
				if (peekPos.z < 0) {
					targetPositionQueue.Dequeue();
					PlayerMove(targetPositionQueue.Peek().transform.position);
				}
			}
	...
	void PlayerMove(Vector3 pos) {
		...
		TweenPlayer.Play(gameObject, new Tween(1.7f).MoveTo(player, pos, EaseType.easeInOutQuad));
	}
	...
	public override GameObject PopTarget(Vector3 pos, Quaternion rot) {
		var square = SquareGenerator.PopSquare(pos, rot, true, Color.white);
		targetPositionQueue.Enqueue(square);
		return square;
	}
}
